...
{
	Header->Magic = GetBigUInt(io);
	Header->nTextures = GetBigUInt(io);
	Header->HeaderSize = GetBigUInt(io);
	return IL_TRUE;
}
	...
	for (n = 0; n < Header.nTextures; n++) {
		...
		switch (DataFormat)
		{
					...
					for (x = 0; x < Image->Width; x += 8) {
						for (yBlock = 0; yBlock < 8; yBlock++) {
							if ((y + yBlock) >= Image->Height) {
								io->seek(io, 8, IL_SEEK_CUR);  // Entire row of pad bytes skipped.
								continue;
							}
							DataOff = Image->Bps * (y + yBlock) + Image->Bpp * x;
							for (xBlock = 0; xBlock < 8; xBlock += 2) {
								BytePixel = io->getc(io);
					...
					for (x = 0; x < Image->Width; x += 8) {
						for (yBlock = 0; yBlock < 4; yBlock++) {
							...
							for (xBlock = 0; xBlock < 8; xBlock++) {
								if ((x + xBlock) >= Image->Width) {
									io->getc(io);  // Skip the pad byte.
									continue;
								}
								Image->Data[DataOff] = io->getc(io);  // Luminance value
								DataOff++;
							}
						}
					}
					...
					for (x = 0; x < Image->Width; x += 4) {
						for (yBlock = 0; yBlock < 4; yBlock++) {
							...
							for (xBlock = 0; xBlock < 4; xBlock += 2) {
								if ((x + xBlock) >= Image->Width) {
									io->seek(io, 2, IL_SEEK_CUR);  // Skip the pad bytes.
									continue;
								}
								Image->Data[DataOff] = io->getc(io);
								Image->Data[DataOff+1] = io->getc(io);
								DataOff += 2;
							}
						}
					}
				...
				for (y = 0; y < Image->Height; y += 4) {
					for (x = 0; x < Image->Width; x += 4) {
						for (yBlock = 0; yBlock < 4; yBlock++) {
							...
							if ((y + yBlock) >= Image->Height) {
								io->seek(io, 16, IL_SEEK_CUR);  // Entire row of pad bytes skipped
								continue;
							}
							...
							for (xBlock = 0; xBlock < 4; xBlock++) {
								if ((x + xBlock) >= Image->Width) {
									io->seek(io, 2, IL_SEEK_CUR);  // Skip pad bytes.
									continue;
								}
								Image->Data[DataOff+3] = io->getc(io);  // Alpha
								Image->Data[DataOff] = io->getc(io);  // Red
								DataOff += 3;
							}
							...
							for (xBlock = 0; xBlock < 4 && x + xBlock < Image->Width; xBlock++) {
								...
								Image->Data[DataOff+1] = io->getc(io);  // Green
								Image->Data[DataOff+2] = io->getc(io);  // Blue
								DataOff += 3;
							}
						}
					}
			...
			case TPL_CI14X2:
				...
				if (io->seek(io, PalOff, IL_SEEK_SET))
					return IL_FALSE;
...
ILboolean TplGetIndexImage(ILimage *Image, ILuint TexOff, ILuint DataFormat)
{
	...
	io->seek(io, 2, IL_SEEK_CUR);  // Do we need to do anything with the 'unpacked' entry?  I see nothing in the specs about it.
				...
				for (x = 0; x < Image->Width; x += 8) {
					for (yBlock = 0; yBlock < 4; yBlock++) {
						...
						for (xBlock = 0; xBlock < 8; xBlock++) {
							...
							Image->Data[DataOff] = io->getc(io);  // Color index
							DataOff++;
						}
					}
				}
