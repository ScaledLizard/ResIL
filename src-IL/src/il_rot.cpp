...
ILboolean iLoadRotInternal(ILimage* image)
{
	...
	io->read(io, Form, 1, 4);
	FormLen = GetBigUInt(io);
	io->read(io, FormName, 1, 4);
	...
	do {
		...
		if (!BaseCreated) {
			if (!il2TexImage(Image, MipWidth, MipHeight, 1, Channels, FormatIL, IL_UNSIGNED_BYTE, NULL))
				return IL_FALSE;
			BaseCreated = IL_TRUE;
		}
		else {
		...
		switch (Format)
		{
			...
			case ROT_DXT1:
				...
				if (io->read(io, CompData, CompSize, 1) != 1)
					return IL_FALSE;
				// ...and decompress it.
			...
			case ROT_DXT3:
				...
				if (io->read(io, CompData, MipSize, 1) != 1)
					return IL_FALSE;
				// ...and decompress it.
	...
	} while (!io->eof(io));  //@TODO: Is there any other condition that should end this?
