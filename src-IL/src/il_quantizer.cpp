...
// palette always has memory space for 256 entries
...
ILboolean Hist3d(ILubyte *Ir, ILubyte *Ig, ILubyte *Ib, ILint *vwt, ILint *vmr, ILint *vmg, ILint *vmb, 
	ILfloat *m2, ILuint size, ILushort	* Qadd)
{
	ILint	ind, r, g, b;
...
ILfloat Var(Box *cube, QuantizerState* qs)
{
	ILfloat dr, dg, db, xx;

	dr = (ILfloat)Vol(cube, qs->mr); 
	dg = (ILfloat)Vol(cube, qs->mg); 
	db = (ILfloat)Vol(cube, qs->mb);
	xx = qs->gm2[cube->r1][cube->g1][cube->b1] 
		-qs->gm2[cube->r1][cube->g1][cube->b0]
		-qs->gm2[cube->r1][cube->g0][cube->b1]
		+qs->gm2[cube->r1][cube->g0][cube->b0]
		-qs->gm2[cube->r0][cube->g1][cube->b1]
		+qs->gm2[cube->r0][cube->g1][cube->b0]
		+qs->gm2[cube->r0][cube->g0][cube->b1]
		-qs->gm2[cube->r0][cube->g0][cube->b0];

	return xx - (dr*dr+dg*dg+db*db) / (ILfloat)Vol(cube, qs->wt);
}
...
ILfloat Maximize(Box *cube, ILubyte dir, ILint first, ILint last, ILint *cut,
				 ILint whole_r, ILint whole_g, ILint whole_b, ILint whole_w, QuantizerState* qs)
{
	...
	base_r = Bottom(cube, dir, qs->mr);
	base_g = Bottom(cube, dir, qs->mg);
	base_b = Bottom(cube, dir, qs->mb);
	base_w = Bottom(cube, dir, qs->wt);
	max = 0.0;
	*cut = -1;

	for (i = first; i < last; ++i) {
		half_r = base_r + Top(cube, dir, i, qs->mr);
		half_g = base_g + Top(cube, dir, i, qs->mg);
		half_b = base_b + Top(cube, dir, i, qs->mb);
		half_w = base_w + Top(cube, dir, i, qs->wt);
...
ILint Cut(Box *set1, Box *set2, QuantizerState* qs)
{
	ILubyte dir;
	...
	whole_r = Vol(set1, qs->mr);
	whole_g = Vol(set1, qs->mg);
	whole_b = Vol(set1, qs->mb);
	whole_w = Vol(set1, qs->wt);

	maxr = Maximize(set1, RED, set1->r0+1, set1->r1, &cutr, whole_r, whole_g, whole_b, whole_w, qs);
	maxg = Maximize(set1, GREEN, set1->g0+1, set1->g1, &cutg, whole_r, whole_g, whole_b, whole_w, qs);
	maxb = Maximize(set1, BLUE, set1->b0+1, set1->b1, &cutb, whole_r, whole_g, whole_b, whole_w, qs);
...
ILimage *iQuantizeImage(ILimage *Image, ILuint NumCols)
{
	...
	if (NumCols <= 256) {
		...
		imemclear(qs.gm2, 33 * 33 * 33 * sizeof(ILfloat));
		imemclear(qs.wt, 33 * 33 * 33 * sizeof(ILint));
		imemclear(qs.mr, 33 * 33 * 33 * sizeof(ILint));
		imemclear(qs.mg, 33 * 33 * 33 * sizeof(ILint));
		imemclear(qs.mb, 33 * 33 * 33 * sizeof(ILint));
                
		if (!Hist3d(Ir, Ig, Ib, (ILint*)qs.wt, (ILint*)qs.mr, (ILint*)qs.mg, (ILint*)qs.mb, (ILfloat*)qs.gm2, 
			size, Qadd))
			goto error_label;

		M3d((ILint*)qs.wt, (ILint*)qs.mr, (ILint*)qs.mg, (ILint*)qs.mb, (ILfloat*)qs.gm2);

		cube[0].r0 = cube[0].g0 = cube[0].b0 = 0;
		cube[0].r1 = cube[0].g1 = cube[0].b1 = 32;
		next = 0;
		for (ILuint i = 1; i < NumCols; ++i) {
			if (Cut(&cube[next], &cube[i], &qs)) { // volume test ensures we won't try to cut one-cell Box */
				vv[next] = (cube[next].vol>1) ? Var(&cube[next], &qs) : 0.0f;
				vv[i] = (cube[i].vol>1) ? Var(&cube[i], &qs) : 0.0f;
			}
			else {
		...
		if (tag == NULL)
			goto error_label;
		for (k = 0; (ILint)k < NumCols; k++) {
			Mark(&cube[k], k, tag);
			weight = Vol(&cube[k], qs.wt);
			if (weight) {
				lut_r[k] = (ILubyte)(Vol(&cube[k], qs.mr) / weight);
				lut_g[k] = (ILubyte)(Vol(&cube[k], qs.mg) / weight);
				lut_b[k] = (ILubyte)(Vol(&cube[k], qs.mb) / weight);
			}
			else {
				// Bogus Box
		...
		il2SetError(IL_INTERNAL_ERROR);  // Can't get much more specific than this.
		goto error_label;
	}
