...
void ILAPIENTRY ilutPushAttrib(ILuint Bits)
{
	...
	if (ilutCurrentPos >= ILUT_ATTRIB_STACK_MAX - 1) {
		...
		il2SetError(ILUT_STACK_OVERFLOW);
		return;
	}
...
void ILAPIENTRY ilutPopAttrib()
{
	if (ilutCurrentPos <= 0) {
		ilutCurrentPos = 0;
		il2SetError(ILUT_STACK_UNDERFLOW);
		return;
	}
...
ILboolean ILAPIENTRY ilutRenderer(ILenum Renderer)
{
	if (Renderer > ILUT_WIN32) {
		il2SetError(ILUT_INVALID_VALUE);
		return IL_FALSE;
	}
	...
	switch (Renderer)
	{
		...
		default:
			il2SetError(ILUT_NOT_SUPPORTED);
	}
